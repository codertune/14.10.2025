    ...
    try {
      ...
      if (result.success) {
        ...
        if (!isLogin) {
          setIsLogin(true);
          setTimeout(() => {
            setMessage('Please check your email and verify your account, then login here.');
          }, 100);
        } else {
          setTimeout(() => {
            console.log('AuthModal: Success, calling onSuccess and onClose');
            console.log('AuthModal: Current user after login:', result.user || 'No user in result');
            if (onSuccess) {
              onSuccess();
            }
            onClose();
            setFormData({ email: '', password: '', name: '', company: '', mobile: '' });
            setMessage('');
          }, 500);
        }
      } else {
            ...
            <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
              ...
              {message && (
                <div className={`mb-4 p-3 rounded-lg text-sm ${
                  message.includes('sent') || message.includes('instructions')
