...
interface CreditSettings {
  ...
  serviceCreditsConfig: { [serviceId: string]: number };
  systemNotification?: {
    enabled: boolean;
    message: string;
...
interface AuthContextType {
  ...
  getServiceCreditCost: (serviceId: string) => number;
  updateServiceCreditCost: (serviceId: string, credits: number) => void;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [creditSettings, setCreditSettings] = useState<CreditSettings>({
    ...
    serviceCreditsConfig: {
      'pdf-excel-converter': 1,
      'ctg-port-tracking': 1,
      'exp-issue': 2,
      'exp-correction': 1.5,
      'exp-duplicate-reporting': 2,
      'exp-search': 0.5,
      'damco-booking': 3,
      'damco-booking-download': 1,
      'damco-fcr-submission': 2,
      'damco-fcr-extractor': 1.5,
      'damco-edoc-upload': 1,
      'hm-einvoice-create': 2,
      'hm-einvoice-download': 1,
      'hm-einvoice-correction': 1.5,
      'hm-packing-list': 1,
      'bepza-ep-issue': 2.5,
      'bepza-ep-submission': 2,
      'bepza-ep-download': 1,
      'bepza-ip-issue': 2.5,
      'bepza-ip-submit': 2,
      'bepza-ip-download': 1,
      'cash-incentive-application': 3,
  ...
  const updateUser = (updates: Partial<User>) => {
    if (user) {
      const localOnlyFields = ['workHistory'];
      const dbFields = ['credits', 'is_admin', 'status', 'name', 'company', 'mobile', 'totalSpent', 'lastActivity'];

      const hasDbUpdates = Object.keys(updates).some(key => dbFields.includes(key));
      const hasLocalUpdates = Object.keys(updates).some(key => localOnlyFields.includes(key));

      if (hasLocalUpdates && !hasDbUpdates) {
      ...
      if (hasDbUpdates) {
        ...
        .then(result => {
          if (result.success) {
            const finalUser = { ...result.user, ...updates };
            setUser(finalUser);
            setUsers(prev => prev.map(u => u.id === user.id ? finalUser : u));
  ...
  const updateCreditSettings = (settings: Partial<CreditSettings>) => {
    ...
    .then(result => {
      if (result.success) {
        ...
        localStorage.setItem('creditSettings', JSON.stringify(result.settings));
      }
    })
    .catch(error => {
  ...
  const deductCredits = (credits: number): boolean => {
    if (user?.isAdmin) {
      console.log('Admin user - unlimited credits, allowing operation');
      return true;
    }
  ...
  const toggleService = (serviceId: string) => {
    ...
    updateCreditSettings({ enabledServices: newEnabledServices });
  };
  ...
  const getServiceCreditCost = (serviceId: string) => {
    return creditSettings.serviceCreditsConfig?.[serviceId] || 1;
  };

  const updateServiceCreditCost = (serviceId: string, credits: number) => {
    const newServiceCreditsConfig = {
      ...creditSettings.serviceCreditsConfig,
      [serviceId]: credits
    };
    
    updateCreditSettings({ serviceCreditsConfig: newServiceCreditsConfig });
  };
  ...
  useEffect(() => {
    if (user && user.id) {
      ...
      fetch(`/api/work-history/${user.id}?page=1&limit=3`)
        .then(response => response.json())
        .then(result => {
