/*
  ...
  1. Column Changes
    - `users.credits` (integer → numeric(10,2))
    - `work_history.credits_used` (integer → numeric(10,2))
    - `automation_jobs.credits_used` (integer → numeric(10,2))

  2. Data Migration
    - Convert existing integer values to decimal format (e.g., 2 → 2.00, 64 → 64.00)
  ...
  3. Important Notes
    ...
    - User credit balances can now handle decimal deductions (e.g., 64.00 - 2.20 = 61.80)
*/
...
-- Update system_settings to ensure credit costs are properly formatted
UPDATE system_settings
SET service_credits_config = jsonb_object_agg(
  key,
  CASE
    WHEN jsonb_typeof(value) = 'number' THEN
      to_jsonb(ROUND((value::text)::numeric, 2))
    ELSE value
  END
)
FROM jsonb_each(service_credits_config);

-- Verify the changes
DO $$
DECLARE
  user_credits_type text;
  wh_type text;
  aj_type text;
BEGIN
  SELECT data_type INTO user_credits_type
  ...
  RAISE NOTICE '  users.credits type: %', user_credits_type;
